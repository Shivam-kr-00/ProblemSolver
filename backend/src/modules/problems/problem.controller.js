...
export const updateProblemStatus = async (req, res, next) => {
  try {
    ...
    const problem = await updateProblemStatusService(problemId);

    res.status(200).json(
      new ApiResponse(200, problem, "Problem status updated successfully")
    );
  } catch (error) {
    next(error);
  }
...
export const addRepoToProblem = async (req, res, next) => {
  try {
    const { problemId } = req.params;
    const { repoUrl } = req.body;

    const problem = await addRepoToProblemService(problemId, repoUrl);

    res.status(200).json(
      new ApiResponse(200, problem, "Repository added successfully")
    );
  } catch (error) {
    next(error);
  }
